 ...
 * - You'll have to manually add processes not recorded on history:
 *   - processes executed in global context
 *   - processes that output new images instead of replacing their target views
 * A TODO comment is generated in the pipeline section to remind this and
 * includes hints on how to code those process executions.
...
Console.writeln("// TODO: Views generated by the pipeline must be declared there (cut and paste from here)");
...
for (viewId of viewArr) {
   ...
   var historyProcessContainer = view.processing; // view's history

   // Record each step in view's history
   for (let i = 0; i < view.historyIndex; i++) { // < historyProcessContainer.length
      var processInstance = historyProcessContainer.at(i);
...
Console.writeln();
Console.writeln("// TODO: you'll have to manually add processes not recorded on history:");
Console.writeln("// - processes executed in global context:");
Console.writeln("//     process.executeGlobal();");
Console.writeln("// - processes that output new images instead of replacing their target views:");
Console.writeln("//     process.executeOn(view);");
Console.writeln();
